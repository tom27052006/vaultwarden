...
mod tests {
    ...
    /// A granular permission is only ever read off a Custom membership. A stale bit on a row that is no
    /// longer Custom -- a role change through the admin panel, a hand-edited database, a partial
    /// rollback -- must grant nothing, and an unknown stored role must not resurrect it either.
    #[test]
    fn custom_permission_flags_are_gated_on_the_custom_role() {
        type Flag = (&'static str, fn(&mut Membership), fn(&Membership) -> bool);
        let flags: [Flag; 6] = [
            ("manageUsers", |m| m.manage_users = true, Membership::has_manage_users),
            ("manageGroups", |m| m.manage_groups = true, Membership::has_manage_groups),
            ("managePolicies", |m| m.manage_policies = true, Membership::has_manage_policies),
            ("editAnyCollection", |m| m.edit_any_collection = true, Membership::has_edit_any_collection),
            ("deleteAnyCollection", |m| m.delete_any_collection = true, Membership::has_delete_any_collection),
            ("accessImportExport", |m| m.access_import_export = true, Membership::has_access_import_export),
        ];

        for (name, set, read) in flags {
            let mut custom = membership(MembershipType::Custom);
            set(&mut custom);
            assert!(read(&custom), "{name} must apply to a Custom member");

            // Owners and Admins hold their authority by role and need none of these bits; a plain User
            // must gain nothing from one.
            for role in [MembershipType::Owner, MembershipType::Admin, MembershipType::User] {
                let mut stale = membership(role);
                set(&mut stale);
                assert!(!read(&stale), "{name} must stay inert on role {}", role as i32);
            }

            let mut unknown = membership(MembershipType::Custom);
            set(&mut unknown);
            unknown.atype = 3;
            assert!(!read(&unknown), "{name} must stay inert on an unknown stored role");
        }
    }

    /// One organization with one collection; `org-b` probes tenant isolation from both directions.
    /// Status 2 is Confirmed and -1 Revoked, atype 2 a plain User.
    #[cfg(sqlite)]
    const COLLECTION_MANAGE_SEED: &str = "
        INSERT INTO collections (uuid, org_uuid, name) VALUES ('col-a', 'org-a', 'A');

        INSERT INTO users (uuid, updated_at) VALUES
            ('u-manage', '2026-01-01 00:00:00'), ('u-write', '2026-01-01 00:00:00'),
            ('u-group-manage', '2026-01-01 00:00:00'), ('u-group-all', '2026-01-01 00:00:00'),
            ('u-revoked', '2026-01-01 00:00:00'), ('u-foreign', '2026-01-01 00:00:00');
        ...
        INSERT INTO users_organizations (uuid, user_uuid, org_uuid, akey, status, atype) VALUES
            ...
            ('m-group-manage', 'u-group-manage', 'org-a', '',  2, 2),
        ...
        INSERT INTO users_collections (user_uuid, collection_uuid, read_only, hide_passwords, manage) VALUES
            ('u-manage', 'col-a', FALSE, FALSE, TRUE), ('u-write', 'col-a', FALSE, FALSE, FALSE),
            ('u-revoked', 'col-a', FALSE, FALSE, TRUE), ('u-foreign', 'col-a', FALSE, FALSE, TRUE);
        ...
        INSERT INTO groups_users (groups_uuid, users_organizations_uuid) VALUES
            ('g-manage', 'm-group-manage'), ('g-all', 'm-group-all');

        INSERT INTO collections_groups (collections_uuid, groups_uuid, read_only, hide_passwords, manage) VALUES
            ('col-a', 'g-manage', FALSE, FALSE, TRUE), ('col-a', 'g-all', FALSE, FALSE, FALSE);
    ";

    /// The predicate every per-collection Manage decision rests on, against the real Diesel queries.
    /// Only a stored `manage` grant counts: a write assignment is access and not management, and the
    /// legacy `groups.access_all` flag is a dynamic read/write reach that must never be read as one.
    /// The grant also has to belong to a confirmed membership of the collection's own organization.
    #[cfg(sqlite)]
    #[test]
    fn explicit_collection_manage_access_accepts_only_real_manage_grants() {
        use crate::db::test_db::{ORG_ACL_SCHEMA, TestDb, block_on};

        async fn may_manage(conn: &DbConn, member_uuid: &str) -> bool {
            Membership::find_by_uuid(&MembershipId::from(member_uuid.to_owned()), conn)
                .await
                .unwrap_or_else(|| panic!("Missing membership {member_uuid}"))
                .has_explicit_collection_manage_access(&CollectionId::from("col-a".to_owned()), conn)
                .await
        }
        ...
        block_on(async {
            ...
            assert!(may_manage(&conn, "m-manage").await);
            assert!(may_manage(&conn, "m-group-manage").await);

            // Write access is not management, and `groups.access_all` is not a manage grant.
            assert!(!may_manage(&conn, "m-write").await);
            assert!(!may_manage(&conn, "m-group-all").await);

            // A grant is only held by a confirmed membership of this collection's organization.
            assert!(!may_manage(&conn, "m-revoked").await);
            assert!(!may_manage(&conn, "m-foreign").await);
        });
    }
}
