...
impl CollectionGroup {
    pub async fn save(&mut self, org_uuid: &OrganizationId, conn: &DbConn) -> EmptyResult {
        ...
        // access. Defense in depth, so no route can create one even if it mis-validates its inputs.
...
impl GroupUser {
    pub async fn save(&mut self, conn: &DbConn) -> EmptyResult {
        ...
        // access-all group. Defense in depth, so no route can create one even if it mis-validates.
