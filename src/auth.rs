...
#[cfg(test)]
mod tests {
    use super::{CollectionManageAccess, collection_delete_access, collection_edit_access, collection_modify_access};
    ...
    /// The three collection decisions this PR separated, which used to be one blanket check.
    ///
    /// `Any` is org-wide authority, `ExplicitManage` means a stored per-collection `manage` row still has
    /// to be proven in the database. Three regressions: a Manage grant doubling as a *delete* permission
    /// (Vaultwarden always reports `limitCollectionDeletion = true`, under which upstream allows only
    /// Owner/Admin and `Delete any collection`); `Manage users` or `Manage groups` alone unlocking
    /// bulk-access, which upstream authorizes against both requirements at once; and an unconfirmed or
    /// uninterpretable membership being handed anything.
    #[test]
    fn collection_authority_separates_edit_bulk_modify_and_delete() {
        use CollectionManageAccess::{Any, Denied, ExplicitManage};

        // Owners and Admins hold every collection authority by role.
        for role in [MembershipType::Owner, MembershipType::Admin] {
            let member = membership(role);
            assert_eq!(collection_edit_access(&member), Any, "role {}", role as i32);
            assert_eq!(collection_modify_access(&member), Any, "role {}", role as i32);
            assert_eq!(collection_delete_access(&member), Any, "role {}", role as i32);
        }

        // A plain User or a flagless Custom member must prove a stored per-collection grant to edit,
        // and can never delete on the strength of one.
        for role in [MembershipType::User, MembershipType::Custom] {
            let member = membership(role);
            assert_eq!(collection_edit_access(&member), ExplicitManage, "role {}", role as i32);
            assert_eq!(collection_modify_access(&member), ExplicitManage, "role {}", role as i32);
            assert_eq!(collection_delete_access(&member), Denied, "role {}", role as i32);
        }

        // `Edit any collection` is org-wide edit, and nothing more.
        let mut edit_any = membership(MembershipType::Custom);
        edit_any.edit_any_collection = true;
        assert_eq!(collection_edit_access(&edit_any), Any);
        assert_eq!(collection_modify_access(&edit_any), Any);
        assert_eq!(collection_delete_access(&edit_any), Denied);

        // `Delete any collection` is org-wide delete, and does not widen edit to org scope.
        let mut delete_any = membership(MembershipType::Custom);
        delete_any.delete_any_collection = true;
        assert_eq!(collection_edit_access(&delete_any), ExplicitManage);
        assert_eq!(collection_delete_access(&delete_any), Any);

        // Bulk-access needs both org-wide member permissions; either one alone falls back to the
        // regular collection-update check.
        let mut manage_users = membership(MembershipType::Custom);
        manage_users.manage_users = true;
        assert_eq!(collection_modify_access(&manage_users), ExplicitManage);
        let mut both = manage_users;
        both.manage_groups = true;
        assert_eq!(collection_modify_access(&both), Any);

        // Status and unknown stored roles fail closed, whatever the flags say.
        for status in
            [MembershipStatus::Invited as i32, MembershipStatus::Accepted as i32, MembershipStatus::Revoked as i32]
        {
            let mut unconfirmed = membership(MembershipType::Owner);
            unconfirmed.status = status;
            assert_eq!(collection_edit_access(&unconfirmed), Denied, "status {status}");
            assert_eq!(collection_delete_access(&unconfirmed), Denied, "status {status}");
        }
        let mut unknown = both;
        unknown.atype = 3;
        assert_eq!(collection_edit_access(&unknown), Denied);
        assert_eq!(collection_modify_access(&unknown), Denied);
        assert_eq!(collection_delete_access(&unknown), Denied);
    }
}
